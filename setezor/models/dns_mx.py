...
class DNS_MX(IDDependent, Base, table=True):
    ...
    target_ip: Optional["IP"]           = Relationship(back_populates="dns_mx_target")
    target_domain: Optional["Domain"]   = Relationship(back_populates="dns_mx_target", sa_relationship_kwargs={"foreign_keys": "[DNS_MX.target_domain_id]"})
    value_domain: Optional["Domain"]    = Relationship(back_populates="dns_mx_value", sa_relationship_kwargs={"foreign_keys": "[DNS_MX.value_domain_id]"})