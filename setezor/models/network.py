...
class Network(IDDependent, Base, table=True):
    ...
    gateway_id: Optional[str]                  = Field(foreign_key="ip.id", sa_column_kwargs={"comment":"Идентификатор шлюза"})
    broadcast_id: Optional[str]                = Field(foreign_key="ip.id", sa_column_kwargs={"comment":"Идентификатор широковещательного IP адреса"})
    network_type_id: str             = Field(default='b271a925283445c5ab60782a42466bfc', foreign_key="d_network_type.id", sa_column_kwargs={"comment":"Тип сети"})
    asn_id: str                      = Field(foreign_key="asn.id", sa_column_kwargs={"comment":"Идентификатор ASN"})
    parent_network_id: Optional[str] = Field(foreign_key="network.id", sa_column_kwargs={"comment":"Идентификатор родительской сети"})

    ips: List["IP"]                     = Relationship(back_populates="network", sa_relationship_kwargs={"foreign_keys": "[IP.network_id]"})
    network_type: "Network_Type"        = Relationship(back_populates="networks")
    asn: "ASN"                          = Relationship(back_populates="networks")
