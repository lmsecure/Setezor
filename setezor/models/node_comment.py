...
class NodeComment(IDDependent, TimeDependent, ProjectDependent, table=True):
    ...
    ip_id: str = Field(foreign_key="ip.id", sa_column_kwargs={"comment":"Идентификатор ip адреса"})
    parent_comment_id: Optional[str] = Field(foreign_key="node_comment.id", sa_column_kwargs={"comment":"Идентификатор родительского коммента"})
