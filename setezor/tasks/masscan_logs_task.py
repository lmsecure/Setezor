...
class MasscanJSONLogTask(MasscanScanTask):
    ...
    def __init__(self, agent_id: int, observer: MessageObserver, scheduler, name: str, task_id: int, input_data: dict, scanning_ip: str, scanning_mac: str, masscan_log_path: str, db: Queries):
        super(MasscanScanTask, self).__init__(agent_id=agent_id,
                                              observer=observer, scheduler=scheduler, name=name)
        self.task_id = task_id
        self._coro = self.run(db=db, task_id=task_id, input_data=input_data, scanning_ip=scanning_ip,
                              scanning_mac=scanning_mac, masscan_log_path=masscan_log_path)
    ...
    async def run(self, db: Queries, task_id: int, input_data: dict, scanning_ip: str, scanning_mac: str, masscan_log_path: str):
        ...
        try:
            ...
            self._write_result_to_db(
                db=db, port_result=ports, link_result=links)
...
class MasscanXMLLogTask(MasscanScanTask):
    ...
    def __init__(self, agent_id: int, observer: MessageObserver, scheduler, name: str, task_id: int, input_data: dict, scanning_ip: str, scanning_mac: str, masscan_log_path: str, db: Queries):
        super(MasscanScanTask, self).__init__(
            agent_id=agent_id, observer=observer, scheduler=scheduler, name=name)
        self.task_id = task_id
        self._coro = self.run(db=db, task_id=task_id, input_data=input_data, scanning_ip=scanning_ip,
                              scanning_mac=scanning_mac, masscan_log_path=masscan_log_path)
