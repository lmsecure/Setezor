window.pendingAgentOverride = null;
const BruteUI = {
  getStyles() {
    return `
      <style>
...
const BruteHandlers = {
  ...
  handlePrefill(instance, rows = [], prefillParams = {}) {
    ...
    if (uniqueTargets.length === 0 && prefillParams?.domain) {
      uniqueTargets.push(String(prefillParams.domain).trim());
    }
...
window.openBruteModal = function () {
  if (document.getElementById('bruteModalWindow')) {
    show_bruteModalWindow();
    return;
  }
  createModal('bruteModalWindow', 'Brute Tool', 'https://help.setezor.net/%D0%98%D0%BD%D1%81%D1%82%D1%80%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D1%8B.html#brute');
  ToolModalBuilder.register('bruteModalWindow', bruteModalConfig);
  show_bruteModalWindow();
};