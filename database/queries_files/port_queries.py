...
class PortQueries(BaseQueries):
    ...
    def create(self, session: Session, ip: str, port: int, protocol: str=None, mac: str=None, service: str=None, product: str=None,
        ...
        new_port_obj = self.model(ip=ip_obj.id, protocol=protocol, port=int(port), service_name=service, product=product, extra_info=extra_info, version=version,
                            os_type=os_type, cpe=cpe, state=state)
        session.add(new_port_obj)
        session.flush()
        self.logger.debug('Created "%s" object with kwargs %s', self.model.__name__, {'ip': ip, 'port': port, 'mac': mac, 'service': service, 'product': product,
                         'extra_info': extra_info, 'version': version, 'os_type': os_type, 'cpe': cpe, 'state': state})
        port_obj = new_port_obj
        return port_obj
    
    @BaseQueries.session_provide
    def get_or_create(self, session: Session, ip: str, port: int, protocol: str=None, mac: str=None, service: str=None, product: str=None,
        ...
        if self.check_exists(session=session, query=port_query):
            if to_update:
                self.update_by_id(id=port_query.first().id, to_update=dict(ip=ip, port=port, protocol=protocol, mac=mac, service=service, product=product,
                      extra_info=extra_info, version=version, os_type=os_type, cpe=cpe, state=state))
            return port_query.first()
        else:
            return self.create(session=session, ip=ip, port=port, protocol=protocol, mac=mac, service=service, product=product,
                      extra_info=extra_info, version=version, os_type=os_type, cpe=cpe, state=state)
    ...
    def get_ports_by_ip(self, session: Session, ip_ids: list) -> dict:
        ...
        return {'headers': df.columns.to_list(), 'data': df.values.tolist()}