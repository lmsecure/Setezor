...
class MAC(IDDependent, Base, table=True):
    ...
    vendor_id: Optional[str]   = Field(foreign_key="d_vendor.id", sa_column_kwargs={"comment":"Идентификатор вендора"})

    ips: List["IP"]              = Relationship(back_populates="mac")
    object: Optional["Object"]   = Relationship(back_populates="macs")
    vendor: "Vendor" = Relationship(back_populates="macs")
