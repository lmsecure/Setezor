...
class Hardware(IDDependent, TimeDependent, table=True):
    ...
    hardware_type_id: str = Field(foreign_key="d_hardware_type.id", sa_column_kwargs={"comment":"Идентификатор типа железки"})

    hardware_type: "Hardware_Type" = Relationship(back_populates="hardwares")
    vendor: "Vendor" = Relationship(back_populates="hardwares")