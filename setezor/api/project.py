...
async def delete_project_by_id(
    ...
    return await ProjectManager.delete_by_id(uow=uow, user_id=user_id, project_id=project.project_id)
...
async def enter_by_invite_token(
    ...
    token_pairs = await AuthService.set_current_project(uow=uow, project_id=project_id, user_id=user_id)
    if not token_pairs:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Incorrect username or password",
