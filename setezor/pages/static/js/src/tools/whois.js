...
const WhoisHandlers = {
  ...
  handleNodeModal(instance, value) {
    ...
    if (typeof value === 'object' && value !== null) {
        domains = value.domains?.filter(d => d && !/^\d+\.\d+\.\d+\.\d+$/.test(d)) || [];
        ip = value.ip || value.label || value.name || '';
        target = domains[0] || ip;
    } else {
        target = String(value || '');
        if (isIPv4(target)) {
            ip = target;
        } else if (WhoisHandlers.isValidDomain(target)) {
            domains = [target];
        }
    }
    ...
    setTimeout(() => {
      if (domains.length > 0) {
          ...
          if (domainInput) domainInput.value = domains[0];
          
          if (domains.length > 1) {
              domains.slice(1).forEach((domain, idx) => {
                  setTimeout(() => {
                      instance.addInputField('domain_only', 'domains', true);
    ...
    if (ips.size > 0) {
      ...
      if (ipInput) {
        ...
        for (let i = 1; i < ipsArray.length; i++) {
          instance.addInputField('ip_only', 'ips', true);
          const container = document.getElementById(`${prefix}ipsInputContainer`);
          if (container) {
            addedGroups.push(container.lastElementChild);
          }
        }
        
        addedGroups.forEach((group, index) => {
          if (group) {
            const input = group.querySelector('.ip');
            if (input) {
              input.value = ipsArray[index + 1];
            }
          }
        });
      }
...
class WhoisScanner {
  ...
  async whoisDomains(targets) {
    ...
    for (const target of targets) {
      ...
      const payload = { ...baseParams, target: d };
...
class WhoisModalBuilder extends ToolModalBuilder {
  constructor(modalId, config) {
    ...
    this.state[`${this.prefix}domains_selectedRowsState`] = {};
    this.state[`${this.prefix}domains_selectedDatabaseTargets`] = {};
    this.state[`${this.prefix}domains_selectedScopeId`] = null;
    
    this.state[`${this.prefix}ips_selectedRowsState`] = {};
    this.state[`${this.prefix}ips_selectedDatabaseTargets`] = {};
    this.state[`${this.prefix}ips_selectedScopeId`] = null;
  }
  ...
  const validTypes = type === 'domains' ? ['domain'] : ['ip'];
  const pfx = `${this.prefix}${type}_`;
  return buildValidatedTextareaHTML(pfx, placeholder, validTypes);
  }
  ...
  initDatabaseTable(type) {
    ...
    const rowsStateKey = `${this.prefix}${type}_selectedRowsState`;
    const targetsStateKey = `${this.prefix}${type}_selectedDatabaseTargets`;
    
    if (!this.state[rowsStateKey]) {
        this.state[rowsStateKey] = {};
    }
    if (!this.state[targetsStateKey]) {
        this.state[targetsStateKey] = {};
    }
    ...
    try {
      ...
      const table = new Tabulator(container, {
        ...
        columns: columns,
        initialSort: [{ column: type === 'domains' ? 'domain' : 'ipaddr', dir: 'asc' }],
        ajaxResponse: (url, params, response) => {
            setTimeout(() => {
                const stateKey = `${this.prefix}${type}_selectedRowsState`;
                ...
                table.getRows().forEach(row => {
                    ...
                    const key = type === 'domains' ? data.domain : data.ipaddr;
                    if (!key) return;
                    
                    if (this.state[stateKey][key]) {
                        row.select();
                    } else {
                        row.deselect();
                    }
      ...
      table.on("rowSelected", (row) => {
          ...
          this.state[rowsStateKey][key] = data;
          
          this.state[targetsStateKey][key] = {
              [type === 'domains' ? 'domain' : 'ip']: key,
              [type === 'domains' ? 'ip' : 'domain']: type === 'domains' ? data.ipaddr : data.domain
          };
      });
      ...
      table.on("rowDeselected", (row) => {
          ...
          delete this.state[rowsStateKey][key];
          delete this.state[targetsStateKey][key];
      });
  ...
  initTextareaDragDrop(type) {
    const pfx       = `${this.prefix}${type}_`;
    const container = document.getElementById(`${pfx}textareaContainer`);
    const textarea  = document.getElementById(`${pfx}textareaInput`);
    const overlay   = document.getElementById(`${pfx}textareaOverlay`);
    if (!container || !textarea || !overlay) return;

    ['dragenter', 'dragover'].forEach(e => {
      container.addEventListener(e, ev => { ev.preventDefault(); ev.stopPropagation(); overlay.classList.remove('d-none'); }, false);
    });
    ['dragleave', 'drop'].forEach(e => {
      container.addEventListener(e, ev => { ev.preventDefault(); ev.stopPropagation(); overlay.classList.add('d-none'); }, false);
    });
    container.addEventListener('drop', e => {
      const file = e.dataTransfer.files[0];
      if (file) {
        const reader = new FileReader();
        reader.onload = ev => { textarea.value = ev.target.result; textarea.dispatchEvent(new Event('input')); };
        reader.readAsText(file);
      }
    }, false);

    this._initTextareaValidator(textarea, container, pfx);
  }

  addInputField(type, subType = null, flag = false) {
  ...
  reset() {
    ...
    ['domains', 'ips'].forEach(type => {
        ...
        this.state[`${this.prefix}${type}_selectedDatabaseTargets`] = {};
        this.state[`${this.prefix}${type}_selectedRowsState`] = {};
    ...
    if (domainsTab) domainsTab.click();
    setTimeout(() => {
        const simpleTab = document.getElementById(`${this.prefix}domains-simple-tab`);
  ...
  async refreshGlobalBars() {
    if (super.refreshGlobalBars) {
        await super.refreshGlobalBars();
    }
    
    await this.refreshDualScopeDropdowns();
  }

  async refreshDualScopeDropdowns() {
      for (const type of ['domains', 'ips']) {
          await this.refreshScopeDropdown(type);
      }
  }

  async refreshScopeDropdown(type) {
    const dropdownMenu = document.getElementById(`${this.prefix}${type}-dropdownMenuForScopeData`);
    const scopeBtn = document.getElementById(`${this.prefix}${type}-ScopeDropdown`);
    ...
    try {
      ...
      if (resp.data.length === 0) {
        dropdownMenu.innerHTML = `
          <li><span class="dropdown-item text-muted">No scopes available</span></li>
        `;
        return;
      }
      ...
      resp.data.forEach((item) => {
        ...
        console.error(`Failed to refresh ${type} scopes:`, err);
        dropdownMenu.innerHTML = `
          <li><span class="dropdown-item text-danger">Error loading scopes</span></li>
        `;
        if (typeof create_toast === "function") {
          create_toast("Warning", `Failed to load ${type} scopes`, "warning");
        }
    }
  }
}

const whoisModalConfig = {
  prefix: 'whois_',
  ...
  onReset(instance) {
    ...
    ['domains', 'ips'].forEach(type => {
      ...
      instance.state[`${prefix}${type}_selectedRowsState`] = {};
      instance.state[`${prefix}${type}_selectedDatabaseTargets`] = {};
    });
  }
};

window.openWhoisModal = function () {
  if (document.getElementById('whoisModalWindow')) {
    show_whoisModalWindow();
    return;
  }
  createModal('whoisModalWindow', 'Whois Tool', 'https://help.setezor.net/%D0%98%D0%BD%D1%81%D1%82%D1%80%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D1%8B.html#whois');
  ToolModalBuilder.instances['whoisModalWindow'] = new WhoisModalBuilder('whoisModalWindow', whoisModalConfig);
  show_whoisModalWindow();
};