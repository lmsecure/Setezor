        ...
        <div class="masscan-switch-row d-flex justify-content-between align-items-center">
          ...
          <div style="width: 55%">
            <datalist id="${prefix}masscanPortsSelectorList">
              ...
              <option value="1-65535" data-i18n="All"></option>
            </datalist>
            <input type="text" 
...
const MasscanHandlers = {
  ...
  handlePrefill(instance, rows = [], prefillParams = {}) {
    ...
    if (prefillParams?.interface_ip_id && prefillParams?.agent_id) {
      window.getInterfaceData(prefillParams.agent_id).then(ifaceData => {
        if (ifaceData?.interfaces) {
          const ifaceObj = ifaceData.interfaces.find(
            i => i.ip_id == prefillParams.interface_ip_id
          );
  ...
  applyPrefillParams(prefix, params) {
    if (params.search_udp_port !== undefined) {
      const protocolId = params.search_udp_port ? `${prefix}protocolUdp` : `${prefix}protocolTcp`;
      const radio = document.getElementById(protocolId);
...
class MasscanScanner {
  ...
  async scanTargets(targets) {
    ...
    for (const target of targets) {
      ...
      const ip = String(target.ip || target.domain || target.url || target).trim();
      if (!ip) continue;
      ...
      if (target.port && target.port.trim()) {
        ...
        if (portsArray.length > 0) {
          portsKey = portsArray.sort().join(','); 
        }
      }
      
      const key = `${ip}:${portsKey}`;
      if (seen.has(key)) continue;
      seen.add(key);
      
      if (portsKey) {
        payload.ports = portsKey;
        payload.ping = false;
      }
      payload.target = ip;
...
const masscanModalConfig = {
  ...
  tabs: [
    ...
    { id: 'textarea', label: i18next.t('Textarea'), placeholder: '192.168.1.1:80\n10.0.0.0/24', textareaValidTypes: ['ip', 'ip_ports', 'network', 'network_ports'] },
  ],
...
window.openMasscanModal = function () {
  if (document.getElementById('masscanModalWindow')) {
    show_masscanModalWindow();
    return;
  }
  createModal('masscanModalWindow', 'Masscan Tool', 'https://help.setezor.net/Инструменты.html#masscan');
  ToolModalBuilder.register('masscanModalWindow', masscanModalConfig);
  show_masscanModalWindow();
};