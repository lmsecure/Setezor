...
class CertInfo:
    ...
    @staticmethod
    def parse_cert(cert: bytes) -> Dict[str, str]:
        ...
        cert_data = {
            ...
            "not-after": CertInfo.convert_timestamp(cert.get_notAfter()),
            "not-before": CertInfo.convert_timestamp(cert.get_notBefore()),
    ...
    @staticmethod
    def get_cert_and_parse(host: str, port: int,certificates_folder:str):
        cert = CertInfo.get_cert(host=host, port=port)
        if not cert:
            return None
        CertInfo.write_to_file(certificates_folder,host,port,cert)
        return CertInfo.parse_cert(cert=cert)

    @staticmethod
    def write_to_file(certificates_folder:str, host:str, port:int,cert:str): # переименовать и поменять формат даты-время
