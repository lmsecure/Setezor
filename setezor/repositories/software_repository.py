...
class SoftwareRepository(SQLAlchemyRepository[Software]):
    ...
    async def for_search_vulns(self, project_id: str, scan_id: str):
        ...
        stmt_l7 = select(Vendor.name, L7Software, self.model).select_from(L7).join(L7Software, L7.id == L7Software.l7_id)\
            .join(self.model, self.model.id == L7Software.software_id)\
            .join(Vendor, self.model.vendor_id == Vendor.id)\
            .filter(L7.project_id == project_id, L7.scan_id == scan_id)\
            .filter(*addition)
        
        l7_soft_with_cpe23 = await self._session.exec(stmt_l7)
        return l4_soft_with_cpe23.all(), l7_soft_with_cpe23.all()
